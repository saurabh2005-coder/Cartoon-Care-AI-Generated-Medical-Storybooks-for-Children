...
export async function downloadStoryPDF(storyId, childName) {
  const token = localStorage.getItem("cc_token");
  const res = await fetch(`/api/stories/${storyId}/download`, {
  ...
  a.href = url;
  a.download = `${childName}_storybook.pdf`.replace(/ /g, "_");
